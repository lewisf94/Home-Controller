/*
 * Music Controller -- ESP-IDF main (HOME ASSISTANT build).
 *
 * Same hardware/UI as cyd/esp-idf/, but the backend is Home Assistant over a
 * local WebSocket (Music Assistant) instead of the Spotify Web API. Wiring
 * only: brings up LCD + touch + LVGL + WiFi + the HA client, then hands the UI
 * surface to ui.c which builds the album browser and now-playing screens.
 *
 ...
 *
 * HA backend (ha_client.c):
 *   ha_task opens a WebSocket to HA, authenticates with a long-lived token,
 *   and subscribes to a Music Assistant media_player entity. HA *pushes*
 *   state (no polling); the ha_client event handler maps it into the shared
 *   track struct and calls ui_set_track_info(). ha_task drains the command
 *   queue into HA service calls and downloads new album art (HA-proxied URL)
 *   into s_art_rgb, publishing via ui_art_refresh().
 */
...
#include "littlefs.h"
#include "secrets.h"
#include "spotify.h"      /* spotify_track_t (shared struct used by ui.c) */
#include "ha_client.h"    /* HA WebSocket backend (replaces spotify.c) */
#include "ui.h"
#include "mcp_input.h"
#include "input.h"
...
/* (album-art de-dup is handled inside ha_client via ha_take_pending_art) */
...
/* HA backend task. Opens the WebSocket (state arrives asynchronously via the
 * ha_client event handler, which calls ui_set_track_info itself), drains the
 * command queue into HA service calls, and downloads new album art off the
 * WebSocket task. Replaces the Spotify poll loop -- HA pushes state, so there
 * is no polling here. */
static void ha_task(void *arg)
{
    ...
    for (;;) {
        ...
        /* New track art? Fetch + decode here, off the WebSocket task. */
        char rel[256];
        if (littlefs_is_mounted() && ha_take_pending_art(rel, sizeof(rel))) {
            char url[320];
            ha_art_full_url(rel, url, sizeof(url));
            size_t bytes = 0;
            if (ha_download_to_file(url, ART_JPEG_PATH, &bytes)) {
                ESP_LOGI(TAG, "art %u bytes -> %s", (unsigned)bytes, ART_JPEG_PATH);
                decode_and_publish_art();
            }
        }
    }
}

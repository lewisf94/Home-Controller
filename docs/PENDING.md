...
Deep-dive queue (one at a time, Lewis's request): **A concurrency — DONE (clean).** **B failure-mode/resilience — DONE:** mostly solid (WiFi reconnect, token refresh, 429 holdoff, 404 wake, OOM-degrades-with-fallbacks all good). Real gap: an unreachable Sonos stalls `spotify_task` on 4 s SOAP timeouts — an auto-probed/restricted Sonos self-heals (failed fetch clears `s_sonos_active` + 10 s backoff, main.c:460), but an EXPLICITLY-picked one (`s_sonos_explicit`) does not, so it stalls ~12 s (3×4 s back-to-back queries) every poll until the user switches device. Minor: no extra backoff on Spotify 5xx (keeps normal poll cadence — not a hot-spin); non-JPEG art body retried not blacklisted; display-init + WiFi-wait return unchecked (hardware-fault only). Two agent "criticals" were FALSE POSITIVES: queue-create `return`s before `xTaskCreate` (no NULL-queue task), and `_post_cmd` NULL-guards the queue. Pending: **C** credential & TLS security posture, **D** long-uptime heap/fragmentation, **E** multi-build drift / `app_core` consolidation, **F** Sonos UPnP/SOAP robustness.

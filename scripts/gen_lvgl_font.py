...
Covers ASCII 0x20-0x7E by default. Use --ranges for a reproducible Unicode
subset without baking an entire font, for example:

    --ranges 0x20-0x7e,0xa0-0x17f,0x2010-0x2026

Anything outside the generated set falls back to --fallback.
"""
...
def main() -> None:
    ...
    ap.add_argument("--ranges", default=None,
                    help="comma-separated inclusive codepoint ranges, e.g. "
    ...
    if args.codepoints is not None:
        ...
        for item in args.ranges.split(","):
            ...
            if "-" in item:
                ...
                start = end = int(item, 0)
            if start > end or start < 0 or end > 0x10FFFF:
                ap.error(f"invalid Unicode range: {item}")
            cp_set.update(range(start, end + 1))
        cps = sorted(cp_set)
    else:
    ...
    rng = (f"codepoints 0x{cps[0]:04X}..0x{cps[-1]:04X} ({len(cps)} sparse)"
           if sparse else f"range 0x{cps[0]:02X}-0x{cps[-1]:02X}")

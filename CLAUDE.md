...
- **Current status:** Phase 2 — Steps 0, 1, 2, 3 and 4 verified on hardware. Display, LVGL, XPT2046 touch and WiFi STA are all up: the red square follows the finger across the whole screen and the boot log prints `wifi connected, IP: x.x.x.x`. WiFi credentials live in `cyd/esp-idf/main/secrets.h` (gitignored; template at `secrets.h.example`). A custom 2 MB factory partition is required (`partitions.csv` + `CONFIG_PARTITION_TABLE_CUSTOM=y`) because the default 1 MB partition overflows once WiFi + mbedTLS link in. Next: Step 5 — HTTPS GET to Spotify (token refresh + `/me/player`). See `docs/ROADMAP.md` Phase 2 table for the full checkpoint list.

...
- **Current status:** Phase 2 — Steps 0, 1, 2, 3, 4 and 5 verified on hardware. Display, LVGL, XPT2046 touch, WiFi STA and Spotify HTTPS are all up: the boot log prints `access token refreshed` followed by `now playing: <track title>` every 5 s while a song plays. WiFi + Spotify credentials live in `cyd/esp-idf/include/secrets.h` (gitignored; template at `include/secrets.h.example`). A custom 2 MB factory partition is required (`partitions.csv` + `CONFIG_PARTITION_TABLE_CUSTOM=y`) because the default 1 MB partition overflows once WiFi + mbedTLS link in. TLS uses the IDF cert bundle (`CONFIG_MBEDTLS_CERTIFICATE_BUNDLE=y`). Next: Step 6 — download `nowplaying.jpg` and display it via `lv_image`. See `docs/ROADMAP.md` Phase 2 table for the full checkpoint list.

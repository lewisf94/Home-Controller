...
3. Check the author identity before the first commit:
   ...
   ```
   A cloud or web session defaults to Claude as the author. Fix an
   incorrect value with this command:
   ```
...
- Do not use an emoji. Do not add a `Co-Authored-By` footer, or a
  "Generated with Claude" footer, or a `https://claude.ai/code/session_...`
  footer.
